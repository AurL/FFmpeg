...
static const GLchar *f_shader_source =
  ...
  "void main() {\n"
  "  vec4 depthsample = texture2D(tex, (texCoord * 0.5 + 0.5));\n"
  "  vec4 K = vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);\n"
  "  vec4 p = mix(vec4(depthsample.bg, K.wz), vec4(depthsample.gb, K.xy), step(depthsample.b, depthsample.g));\n"
  "  vec4 q = mix(vec4(p.xyw, depthsample.r), vec4(depthsample.r, p.yzx), step(p.x, depthsample.r));\n"
  "  float d = q.x - min(q.w, q.y);\n"
/*  "  float d = 1.0;\n"*/
/*  "  vec4 q = texture2D(tex, (texCoord * 0.5 + 0.5));\n"*/
  "  vec3 result = vec3(abs(q.z + (q.w - q.y) / (6.0 * d + 0.00001)), d / (q.x + 0.00001), q.x);\n"
  "  float val = result.b > 0.5 + 0.01 ? 1.0  - (result.r) : 0.0;\n"
  "  vec3 color = vec3(val, val, val);\n"
